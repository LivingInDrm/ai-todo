...
- 修改步骤：
  ...
  2. 确认 无编译错误，并在 PR 模板中加入"视觉回归核对"清单。⚠️ (TypeScript errors need fixing)
...
- 风险与规避：无。
- 预计工作量：S（<0.5d）。
- 状态：分支已创建，TypeScript错误修复中（24个null类型相关错误）
...
- 修改步骤：
  ...
  2. 在 `lib/theme/index.ts` 确保 re-export 新增模块；`Theme` 类型纳入新增 token。
  3. 检查 `lib/theme/sizing.ts` 是否满足后续需要（FAB、checkbox、swipeAction、avatar 等已具备）。若需，扩展 `sizing.border` 或新增 `layout.*`（可选）。
...
- 修改步骤：
  ...
  3. 若需要额外层级，在 `elevationPresets` 中新增并使用。
...
- 修改步骤：
  ...
  3. 其它 `#000/#FFF` 硬编码点替换（参照 `issues.md` 列举）。
...
- 修改步骤（分文件执行，按行内注释补足）：
  ...
  3. `app/settings.tsx`：section/card/header 的 `padding/margin/fontSize/borderRadius` 全部替换为 token。
  4. 组件：
...
## 任务8｜引入 `ResponsiveContainer` 到页面层
- 依赖：任务2、任务6
...
- 修改步骤：
  1. 用 `useScreenSize()` 与屏幕高度计算 snapPoints：
     ...
     - regular: `['60%', 480]`
     - wide: `['50%', 560]`
  2. 将 Header/Footer 的 padding、divider、handleIndicator 全部走 token；必要时抽象 `Sheet.Header/Content/Footer` 子组件（可在 `@ui/Sheet.tsx` 扩展）。
